# Privacy Policy Changelog

This file records changes to the Entovist Privacy Policy.

| Version | Effective Date | Markdown | PDF | Description |
| ------- | -------------- | -------- | --- | ----------- |
| 1.2.0 | 2026-06-28 | [View](archive/Entovist-Privacy-Policy-v1.2.0.md) | [View](archive/Entovist-Privacy-Policy-v1.2.0.pdf) | Added Amazon S3 for uploaded documents, user profile images, and backup storage. |
| 1.1.0 | 2026-06-22 | [View](archive/Entovist-Privacy-Policy-v1.1.0.md) | [View](archive/Entovist-Privacy-Policy-v1.1.0.pdf) | Added Cookiebot by Usercentrics consent management, Google Analytics 4, Google Consent Mode v2, prior-consent activation, and consent withdrawal details. |
| 1.0.0 | 2026-06-15 | [View](archive/Entovist-Privacy-Policy-v1.0.0.md) | [View](archive/Entovist-Privacy-Policy-v1.0.0.pdf) | Initial public release. |

## Versioning Policy

- Major version changes (2.0, 3.0, etc.) indicate substantial legal or operational changes.
- Minor version changes (1.1, 1.2, etc.) indicate additions or clarifications.
- Patch versions (1.0.1, 1.0.2, etc.) indicate corrections without material legal impact.
